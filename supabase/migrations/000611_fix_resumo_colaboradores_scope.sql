-- 00061_fix_resumo_colaboradores_scope.sql
-- Quando p_admin_scope está ativo (admin não-gestor), a função
-- calcular_resumo_colaboradores_ordens fazia LEFT JOIN com TODOS os admins
-- ativos, retornando N linhas com total=0 para outros admins.
-- O frontend filtrava total<=0, mas o desperdício de processamento e o
-- risco de regressão foram eliminados restringindo o CTE admins ao escopo.
...
AS $$
  ...
  -- Quando p_admin_scope está ativo, restringe ao próprio admin para evitar
  -- retornar N linhas com total=0 para todos os outros colaboradores.
  admins AS (
  ...
  sem_responsavel AS (
    ...
    WHERE responsavel_atual_id IS NULL
      AND p_admin_scope IS NULL   -- sem responsável só aparece na visão gestor
  )
  SELECT *
  FROM por_admin
  UNION ALL
...
COMMENT ON FUNCTION public.calcular_resumo_colaboradores_ordens IS
  ...
  'Quando p_admin_scope está ativo, retorna apenas o próprio admin (visão individual). '
  'Sem p_admin_scope (gestor), retorna todos os admins ativos + "Sem responsável".';
