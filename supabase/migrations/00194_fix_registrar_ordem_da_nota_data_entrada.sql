-- 00194_fix_registrar_ordem_da_nota_data_entrada.sql
--
-- Problema:
-- registrar_ordem_da_nota() (00193) inseria linha em ordens_notas_acompanhamento
-- sem preencher data_entrada. A view vw_ordens_notas_painel filtra
-- WHERE o.data_entrada IS NOT NULL, entao a ordem ficava invisivel no painel.
--
-- Fix:
-- 1. Adiciona data_entrada = now() no INSERT de registrar_ordem_da_nota()
-- 2. Backfill: preenche data_entrada para linhas criadas pela 00193 que ficaram NULL
...
BEGIN
  ...
  ELSE
    ...
    SET
      ...
      data_entrada = COALESCE(ordens_notas_acompanhamento.data_entrada, now()),
...
-- Backfill: preenche data_entrada nas linhas criadas pela 00193 que ficaram NULL
UPDATE public.ordens_notas_acompanhamento
SET data_entrada = ordem_detectada_em
WHERE data_entrada IS NULL
  AND ordem_detectada_em IS NOT NULL;

DO $$
DECLARE v_count INTEGER;
BEGIN
  GET DIAGNOSTICS v_count = ROW_COUNT;
  RAISE NOTICE '00194 backfill: % linhas com data_entrada preenchida', v_count;
END;
$$;
