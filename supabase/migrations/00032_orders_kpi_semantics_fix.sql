-- 00032_orders_kpi_semantics_fix.sql
-- Corrige semantica dos KPIs para separar categorias principais e indicadores complementares.
...
AS $$
  ...
  SELECT json_build_object(
    ...
    'concluidas', COUNT(DISTINCT ordem_id) FILTER (WHERE status_ordem = 'concluida')::INTEGER,
    ...
    'atrasadas', COUNT(DISTINCT ordem_id) FILTER (
      WHERE semaforo_atraso = 'vermelho'
        AND status_ordem IN ('aberta', 'em_tratativa', 'desconhecido')
    )::INTEGER,
