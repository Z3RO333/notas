-- 00260_fix_distribuir_notas_criticos_ocultar.sql
--
-- Corrige o tipo do parâmetro p_sync_id (era INTEGER na 00259, deve ser UUID).
-- Adiciona lógica: quando especialidade = 'criticos' e nenhum admin disponível,
-- oculta a nota (exclui_cockpit = true) em vez de redistribuir para geral.

DROP FUNCTION IF EXISTS public.distribuir_notas(INTEGER);
...
BEGIN
  FOR v_nota IN
    ...
    WHERE n.status = 'nova'
      ...
      AND NOT EXISTS (
        SELECT 1 FROM public.notas_status_sap_aux aux
  ...
  LOOP
    ...
    IF v_especialidade = 'excluir' THEN
      ...
      SET exclui_cockpit = true, updated_at = now()
      WHERE id = v_nota.id;
      CONTINUE;
    END IF;
    ...
    IF v_especialidade IN ('cd_manaus', 'cd_taruma', 'criticos', 'cd_manaus_equip') THEN
      WITH stats AS (
        SELECT
          a.id, a.nome, a.meta_semanal,
          (SELECT COUNT(*)::INTEGER FROM public.vw_notas_sem_ordem v WHERE v.administrador_id = a.id) AS open_count,
          (SELECT COALESCE(MAX(n_ld.distribuida_em), NULL)
           ...
           WHERE n_ld.administrador_id = a.id
             ...
             AND NOT EXISTS (
               SELECT 1 FROM public.notas_status_sap_aux aux_ld
               WHERE aux_ld.numero_nota_norm = COALESCE(NULLIF(LTRIM(BTRIM(n_ld.numero_nota), '0'), ''), '0')
          ...
          (SELECT COUNT(*) FROM public.notas_manutencao n7
           WHERE n7.administrador_id = a.id
             ...
             AND NOT EXISTS (
               SELECT 1 FROM public.notas_status_sap_aux aux7
               WHERE aux7.numero_nota_norm = COALESCE(NULLIF(LTRIM(BTRIM(n7.numero_nota), '0'), ''), '0')
                 AND aux7.status_canonico IN ('CANCELADA', 'VIROU_ORDEM')
             )
          ) AS notas_7d,
          (SELECT COUNT(*) FROM public.ordens_notas_acompanhamento o7
           WHERE o7.administrador_id = a.id AND o7.ordem_detectada_em >= NOW() - INTERVAL '7 days') AS ordens_7d,
        ...
        WHERE a.role = 'admin' AND a.ativo = true AND a.em_ferias = false
          AND a.especialidade = v_especialidade
      )
      SELECT id, open_count, ultima_distribuicao,
        (meta_semanal - (notas_7d * 1.0 + ordens_7d * 0.5)
         - CASE WHEN open_count <= 10 THEN 0 WHEN open_count <= 20 THEN 8
                WHEN open_count <= 30 THEN 20 ELSE 40 END
         - ordens_ativas * 0.5) AS prioridade
      INTO v_admin
      FROM stats
      ...
      IF NOT FOUND THEN
        -- Sem especialista criticos: oculta em vez de redistribuir para geral
        IF v_especialidade = 'criticos' THEN
          UPDATE public.notas_manutencao
          SET exclui_cockpit = true, updated_at = now()
          WHERE id = v_nota.id;
          CONTINUE;
    ...
    IF NOT FOUND OR v_especialidade NOT IN ('cd_manaus', 'cd_taruma', 'criticos', 'cd_manaus_equip') THEN
      WITH stats AS (
        SELECT a.id, a.nome, a.meta_semanal,
          (SELECT COUNT(*)::INTEGER FROM public.vw_notas_sem_ordem v WHERE v.administrador_id = a.id) AS open_count,
          (SELECT COALESCE(MAX(n_ld.distribuida_em), NULL)
           FROM public.notas_manutencao n_ld
           WHERE n_ld.administrador_id = a.id
          ...
          (SELECT COUNT(*) FROM public.notas_manutencao n_hj
           WHERE n_hj.administrador_id = a.id
             AND n_hj.distribuida_em >= CURRENT_DATE
             AND COALESCE(n_hj.exclui_cockpit, false) = false
             AND NOT EXISTS (
               SELECT 1 FROM public.notas_status_sap_aux aux_hj
               WHERE aux_hj.numero_nota_norm = COALESCE(NULLIF(LTRIM(BTRIM(n_hj.numero_nota), '0'), ''), '0')
                 AND aux_hj.status_canonico IN ('CANCELADA', 'VIROU_ORDEM')
             )
          ) AS notas_hoje
        FROM public.administradores a
        WHERE a.role = 'admin' AND a.ativo = true AND a.recebe_distribuicao = true AND a.em_ferias = false
          AND a.especialidade = v_especialidade
      ),
      mediana AS (
        SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY open_count) AS mediana_abertas FROM stats
      )
      SELECT s.id, s.open_count, s.ultima_distribuicao,
        ((s.meta_semanal - s.notas_7d) - s.notas_hoje * 8
         - GREATEST(0, s.open_count::numeric - m.mediana_abertas) * 3) AS prioridade
      INTO v_admin
    ...
    VALUES (
      v_nota.id, 'administrador_id', NULL, v_admin.id::TEXT,
