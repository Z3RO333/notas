-- 00110_revert_resumo_colaboradores_status_filter.sql
-- Reverte 00109: restaura o filtro de status ativo em calcular_resumo_colaboradores_ordens.
-- O card deve contar apenas ordens em ('aberta', 'em_tratativa', 'desconhecido').
-- Concluídas/canceladas não devem inflar o total do card (ficam apenas nos KPIs).
-- A consistência entre card e lista é garantida no frontend (orders-workspace.tsx).
