-- 00221_excluir_preventiva_1500h.sql
--
-- Notas com "PREVENTIVA DE 1.500 HORAS" não devem aparecer em lugar nenhum:
-- nem no painel de notas, nem no cockpit, nem no copilot.
--
-- Mecanismo:
--   1. Regra de distribuição com especialidade='excluir' e pula_cockpit=true
--   2. distribuir_notas() detecta 'excluir' e apenas seta exclui_cockpit=true
--      sem atribuir a ninguém e sem registrar no log
--   3. Backfill das 47 notas existentes

-- ============================================================
-- 1. Regra de distribuição
-- ============================================================
DELETE FROM public.regras_distribuicao
WHERE UPPER(palavra_chave) = 'PREVENTIVA DE 1.500 HORAS';

INSERT INTO public.regras_distribuicao (palavra_chave, especialidade, pula_cockpit)
VALUES ('PREVENTIVA DE 1.500 HORAS', 'excluir', true);

-- ============================================================
-- 2. distribuir_notas() com suporte a especialidade='excluir'
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Notas marcadas para exclusão total: seta flag e pula sem atribuir
...
-- ============================================================
-- 3. Backfill: marcar as 47 notas existentes como exclui_cockpit
-- ============================================================
UPDATE public.notas_manutencao
SET exclui_cockpit = true,
    updated_at     = now()
WHERE exclui_cockpit = false
  AND UPPER(descricao) LIKE '%PREVENTIVA DE 1.500 HORAS%';

-- Remove do cockpit as notas agora marcadas
