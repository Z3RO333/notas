-- 00178_add_status_ativas_filter.sql
-- Adiciona suporte ao filtro p_status = 'ativas' em filtrar_ordens_workspace.
-- 'ativas' exclui concluídas (concluida) e canceladas (cancelada), mostrando apenas
-- ordens que ainda precisam de acompanhamento.
-- Isso permite que o painel de ordens use 'ativas' como padrão, com concluídas
-- acessíveis via filtro de status explícito.
...
AS $$
  ...
  WHERE
    ...
    AND (
      ...
      OR p_status = 'todas'
      OR (
        -- 'ativas' = tudo exceto concluídas e canceladas
        p_status = 'ativas'
