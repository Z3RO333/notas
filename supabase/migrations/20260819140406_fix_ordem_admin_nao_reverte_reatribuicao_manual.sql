-- 20260819135854_fix_ordem_admin_nao_reverte_reatribuicao_manual.sql
--
-- Bug root cause (reincidencia do mesmo padrao ja visto no vazamento do Mayky,
-- 00311_fix_mayky_leak_notas_admin_root_cause.sql): registrar_ordens_por_notas()
-- e registrar_ordem_da_nota() sempre priorizavam o admin da NOTA sobre o admin
-- ja setado na ORDEM (CASE WHEN v_nota.administrador_id IS NOT NULL THEN ...
-- primeiro). Toda reatribuicao manual de uma ordem (ex.: gestor move ordem do
-- Wanderlucio pra Paula) cria uma divergencia entre o admin da nota e o admin
-- da ordem. Essa mesma divergencia era usada como gatilho de reprocessamento
-- (`n.administrador_id IS NOT NULL AND o.administrador_id IS DISTINCT FROM
-- n.administrador_id`), entao no proximo sync do SAP (disparado por qualquer
-- alteracao na ordem no SAP GUI — mudanca de preco, status, etc.) a nota
-- reentrava no loop e a funcao revertia a ordem de volta pro admin da nota,
-- desfazendo silenciosamente a reatribuicao manual. Caso relatado: ordem
-- 5233876 (nota 10180700), reatribuida manualmente pra Paula e revertida pra
-- Wanderlucio a cada sync subsequente.
--
-- Fix:
-- 1. Inverte a prioridade quando a ordem ja existe: admin da ORDEM prevalece
--    sobre o admin da NOTA (alinhado com a regra "nao auto-redistribuir
--    ordens existentes" — RUNBOOK 5.1). Nota so decide o admin quando a
--    ordem ainda nao tem um.
-- 2. Remove a condicao de reprocessamento por divergencia de admin no WHERE
--    do loop — ela so existia para forcar essa reversao indevida; sem ela,
--    ordens reatribuidas manualmente param de reentrar no loop a toa.
-- 3. Override CD MANAUS + equipamento -> Daniel continua valendo como
--    excecao deliberada (nao mexido aqui).

-- ============================================================
-- 1. registrar_ordens_por_notas (base: 00300, com prioridade invertida)
-- ============================================================
...
BEGIN
  FOR v_nota IN
    ...
    WHERE COALESCE(NULLIF(BTRIM(n.ordem_sap), ''), NULLIF(BTRIM(n.ordem_gerada), '')) IS NOT NULL
      AND (
        ...
        -- Reprocessa apenas quando a ordem ainda nao tem admin (nota entao
        -- pode preenche-lo). Nao reprocessa mais so por causa de divergencia
        -- entre admin da nota e admin ja setado na ordem (isso e reatribuicao
        -- manual legitima, nao um erro a corrigir).
        OR (n.administrador_id IS NOT NULL AND o.administrador_id IS NULL)
  ...
  LOOP
    ...
    ELSE
      -- Para ordens existentes: admin da ORDEM prevalece (reatribuicao manual
      -- nao pode ser revertida pelo sync). So cai pro admin da nota quando a
      -- ordem ainda nao tem admin nenhum.
...
COMMENT ON FUNCTION public.registrar_ordens_por_notas(uuid) IS
  ...
  'Convergente desde 00300. Desde 20260819135854: admin da ORDEM prevalece '
  'sobre admin da NOTA quando ja setado — reatribuicao manual de ordem nao e '
  'mais revertida pelo sync.';
...
BEGIN
  ...
  WHERE n.id = p_nota_id
    ...
    AND (
      o.id IS NULL
      OR o.nota_id IS DISTINCT FROM n.id
      -- Reprocessa apenas quando a ordem ainda nao tem admin.
      OR (n.administrador_id IS NOT NULL AND o.administrador_id IS NULL)
      OR n.status IN ('nova', 'em_andamento', 'encaminhada_fornecedor')
      OR COALESCE(NULLIF(BTRIM(n.ordem_gerada), ''), '') = ''
    );
